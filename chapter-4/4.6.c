...
#define VAR		'2'	/* use variable */
#define VAR_PR	11	/* num of 'l' in alphabet */
int getarg(char *s);
...
int main(void)
{
	...
	char *cmd_table[] = {
		"pr",		/* print top of stack, not remove */
		"du",		/* dublicate top */
		"sw",		/* swap two tops */
		"cl",		/* clear all stack */
		"sin",		/* sin(x) */
	...
	double var[26];	/* for a-z, p is special (last printed num) */
	...
	while ((type = getarg(s)) != EOF) {
		switch (type) {
			...
			case CMD:	/* command -> find in table and execute */
				...
				if (!strcmp(s, "pr")) {		/* print top */
					tmp = pop();
					printf("> %lf\n", tmp);
					push(tmp);
					var[VAR_PR] = tmp; /* update last printed */
				} else if (!strcmp(s, "du")) {	/* dublicate top */
					tmp = pop();
					push(tmp);
					push(tmp);
				} else if (!strcmp(s, "sw")) {	/* swap two tops */
					tmp = pop();
					tmp2 = pop();
					push(tmp);
					push(tmp2);
				} else if (!strcmp(s, "cl")) /* clear stack */
					clear_stack();
			...
			case '%':	/* a b % -> (a % b) */
				...
				break;
			case '\n':	/* show result */
				var[VAR_PR] = pop();	/* update last printed */
				printf("> %lf\n", var[VAR_PR]);
				break;
...
int getarg(char *s)
{
	 ...
	 * - a-z -- command start or variable, in the var case
	 *   = may be before
	 */

	if (!(isdigit(c) || c == '.' || c == '-' || c == '=' || islalpha(c)))
		return c;		/* first case -> ret op */

	/* command or variable */
	if (islalpha(c)) {
		*s++ = c;	/* write var/1st command letter anyway */
		c = getch(&b);
		if (!islalpha(c)) {	/* variable */
			b = c;
			*s = '\0';
			return VAR;
		} else {		/* command */
	...
	/* set variable */
		...
		c = getch(&b);
		if (islalpha(c)) {	/* variable specified: a-z */
			*s++ = c;
		} else {		/* var isn't specified: a by default */
			*s++ = 'a';
			b = c;		/* return last symbol */
		}
		*s = '\0';		/* not needed, since 1 letter, but caller
					   may use strcmp() etc. */
		return '=';
